...
class Selenium_Edge:
    ...
    def create_driver(self):
        ...
        s = Service('../NER相关/drive/msedgedriver.exe')
    ...
    def update_driver():
        ...
        driver_version = get_driver_version("../NER相关/drive/msedgedriver.exe")
        ...
        if (browser_version != driver_version):
            ...
            # 构建目标文件夹路径
            target_directory = os.path.join(current_directory, "../NER相关/drive")
            # 将驱动程序文件夹移动到目标文件夹
            if os.path.exists(target_directory):
                shutil.rmtree(target_directory)
    ...
    @locate_element
    def get_element_attribute(self, element, attribute='textContent'):
        try:
            # 'textContent' 标签内的文字
            # 'innerHTML'   标签的html
    ...
    @locate_element
    def get_element_text_json(self, element):
        try:
            text = element.text  # Keep the original text without stripping whitespace
            return text if text else "None"  # If text is not empty, return it; otherwise, return "None"
        except Exception as e:
            return default_value  # In case of an exception, return a default value
    ...
    @locate_element
    def send_files(self,element, folder_path,*args, **kwargs):
        pdf_files = [os.path.join(folder_path, f) for f in os.listdir(folder_path) if f.endswith('.pdf')]
        if pdf_files:
            # pdf_file_path = os.path.join(folder_path, pdf_files)  # Select the first PDF file
            element.send_keys(pdf_files)
